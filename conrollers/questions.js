...
const error = (quiz) => {
    return((TIME_WEIGHT * abs(total(quiz) - TIME)) / TIME + (DIFFICULTY_WEIGHT * abs(average(quiz) - DIFFICULTY)) / DIFFICULTY);
};
...
exports.createQuestionBank = asyncHandler(async (req, res, next) => {
    ...
    const questionBank = await QuestionBank.create({name: "Chemistry", user: req.user.id});

    const chapter1 = await Chapter.create({name: "Chapter 1", questionBank: questionBank});

    const chapter2 = await Chapter.create({name: "Chapter 2", questionBank: questionBank});

    const chapter3 = await Chapter.create({name: "Chapter 3", questionBank: questionBank});

    const chapter4 = await Chapter.create({name: "Chapter 4", questionBank: questionBank});

    const chapter5 = await Chapter.create({name: "Chapter 5", questionBank: questionBank});

    const chapter6 = await Chapter.create({name: "Chapter 6", questionBank: questionBank});

    req.body.forEach((question) => {
        let chapter = chapter1;
        if (question.chapter == 2) 
            chapter = chapter2;
         else if (question.chapter == 3) 
            chapter = chapter3;
         else if (question.chapter == 4) 
            chapter = chapter4;
         else if (question.chapter == 5) 
            chapter = chapter5;
         else if (question.chapter == 6) 
            chapter = chapter6;
...
exports.getAllQuestionBanks = asyncHandler(async (req, res, next) => {
    const questionBanks = await QuestionBank.find().populate({path: "user"}).populate({path: "chapters", select: "name"});
...
exports.getUserChapterQuestionBanks = asyncHandler(async (req, res, next) => {
    ...
    const questionBanks = await QuestionBank.findOne({user: req.user.id, _id: req.params.id});
...
exports.getUserQuestions = asyncHandler(async (req, res, next) => {
    console.log(req.user.id);
    const questionBanks = await QuestionBank.findOne({user: req.user.id, _id: req.params.questionBankId});
...
exports.generateQuiz = asyncHandler(async (req, res, next) => {
    ...
    for (let key in req.body.chapters) 
        if (req.body.chapters.hasOwnProperty(key) && req.body.chapters[key] === true) 
            chapters.push(key);
    ...
    if (req.body.quiz == true) {
        ...
        let userCourse = await UserCourse.findOne({user: req.user.id, questionBank: req.body.questionBank});
        ...
        let userChapter = await UserChapter.findOne({userCourse: userCourse, chapter: chapter});
  ...
  population.forEach((pop) => {
    ...
    while (maxFitness < THRESS_HOLD) {
        for (let i = 0; i < population.length; i++) { // First Parent
            ...
            if (time_test - a[a.length - 1].time > a[a.length - 1].time - a[a.length - 2].time) {
                a.pop();
            }
            population[i] = a;
            // Mutation Operator
    ...
    return res.status(200).json({success: true, data: population[maxFitnessIndex], total: population[maxFitnessIndex].length});
});
...
exports.submitQuiz = asyncHandler(async (req, res, next) => {
    const userCourse = await UserCourse.findOne({user: req.user.id, questionBank: req.body.questionBank});
    const userChapter = await UserChapter.findOne({userCourse: userCourse, chapter: req.body.chapter});
    if (! userChapter) {
